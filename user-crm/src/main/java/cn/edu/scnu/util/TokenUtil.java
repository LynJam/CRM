...
public class TokenUtil {
   ...
   * token 过期时间
   */
  private static final long TOKEN_EXPIRE_TIME = TimeUnit.MILLISECONDS.convert(20,
      TimeUnit.MINUTES);

  /**
   * refresh_token 过期时间
   */
  public static final long REFRESH_TOKEN_EXPIRE_TIME = TimeUnit.MILLISECONDS.convert(10,
      TimeUnit.DAYS);
  ...
  private static String generateAnyToken(String userId, long expireTime, String tokenSecret) {
    ...
    String token = Jwts.builder().setSubject(userId).setIssuedAt(now)
        .setExpiration(expireData).signWith(SignatureAlgorithm.HS512, tokenSecret)
        .compact();
    return token;
  }
  ...
  private static String getUserIdByAnyToken(String token, String tokenSecret) {
    ...
    try {
      Claims claims = Jwts.parser().setSigningKey(tokenSecret).parseClaimsJws(realToken)
          .getBody();
